            ...
            if (result.d.Result) {
                ...
                for (var i = 0; i < parsedTest.length; i++) {
                    ...
                    tabla_lista.row.add([
                        ...
                        '<center>' + stado + '</center>',
                        '<center><button type="button" class="btn btn-info " onclick="ver_capacidad(\'' + parsedTest[i]['id'] + '\',\'' + parsedTest[i]['descripcion'] + '\',\'' + parsedTest[i]['estatus'] + '\')"><i class="fa fa-edit"></i></button></center>'
                    ]).draw();
                }
            }
        },
...
function guardar() {
    ...
    if (!editando) {//GUARDAR
        urlS = "CapacidadTanque.aspx/guardar_capacidad";
    } else { //EDITANDO
        urlS = "CapacidadTanque.aspx/actualizar_capacidad";
    }
    var datas = {
...
function ver_capacidad(id, descripcion, estatus) {
    $("#form1")[0].reset();
    editando = true;
    id_editando = id;
