...
namespace wa_sicove.sicovegas
{
    public partial class Unidades : System.Web.UI.Page
    {
        ...
        public class objU
        {
            ...
            public string comodatarioS { get; set; }
            public int comodatario { get; set; }
            ...
            public int? id_estatus_instalacion { get; set; }
        ...
        public static ajaxResponse traer_u(string empresa, string ncontrato, string neconomico, string comodatario, string placa)
        {
            ...
            try
            {
                ...
                /* var query = from u in context.si_unidades
                             ...
                             join co in context.si_comodatarios on u.id_comodatario equals co.id_comodatario
                             where gi.gi_autoconsumo == 0
                             select new { unidades = u, comodatarios = co } ;
                */

                var query = context.SPTRAERU(empresa, ncontrato, neconomico, comodatario, placa);
                ...
                foreach (var items in query.ToList())
                {
                    ...
                    dato.comodatario = (items.id_comodatario != null) ? (int)items.id_comodatario : -1;
                    dato.comodatarioS = (items.co_nombre != null) ? items.co_nombre : string.Empty;
                    ...
                    dato.empresa = items.emp_nombre;
                    dato.giro = items.gi_descripcion;
                    dato.placa = items.un_placa;
                    dato.direccion = items.un_direccion;
                    dato.telefono = items.un_telefono;
                    dato.fecha_instalacion = items.un_fecha_instalacion;
                    dato.id_estatus_instalacion = items.estatus_instalacion;
                    _list.Add(dato);
                }

                var jsonSerialiser = new JavaScriptSerializer();
                jsonSerialiser.MaxJsonLength = 1999999999;
        ...
        public static ajaxResponse traer_unid(int id)
        {
            ...
            try
            {
                ...
                foreach (var items in query)
                {
                    ...
                    dato.codigo_barra = (items.un_codigo_barra != null) ? items.un_codigo_barra : string.Empty;
                    dato.telefono = (items.un_telefono != null) ? items.un_telefono : string.Empty;
                    dato.direccion = (items.un_direccion != null) ? items.un_direccion : string.Empty;
                    ...
                    dato.id_lider = (items.id_lider != null) ? (int)items.id_lider : -1;
                    dato.lider = (items.si_lideres != null) ? (items.si_lideres.li_nombre + " " + items.si_lideres.li_apellido) : string.Empty;
                    ...
                    dato.id_base = (items.id_base != null) ? (int)items.id_base : -1;
                    dato.id_estatus_instalacion = (items.estatus_instalacion != null) ? (int)items.estatus_instalacion : -1;
        ...
        [WebMethod(EnableSession = true)]
        public static ajaxResponse guardar_u(string numero_economico, string numero_serie, int comodatario, int bases, int estatus_instalacion, DateTime fecha_instalacion, string codigo_barra,
            string telefono, string direccion, int anio, string placa, string observacion, int estatus, int id_empresa, int id_giro,
            int id_marca, int id_modelo, int id_persona, int id_cilindro, int? id_lider, string vaporizador, string riel, string centralita, string observacionK,
            int? id_modeloK, int? id_cilindroKit, string[] numSerieT, DateTime[] fechaFT, string[] multvT, int[] marcaT, int[] tipoT, int[] capacidadT, int id, int kilometraje, int tMantenimiento, int tTiempo)
        {
            ajaxResponse Response = new ajaxResponse();
            try
            {
                id_modeloK = id_modeloK == null ? 1 : id_modeloK;
                id_cilindroKit = id_cilindroKit == null ? 1 : id_cilindroKit;

                sicoveDBDataContext context = new sicoveDBDataContext();
                var query = from u in context.si_unidades
                            where u.un_codigo_barra == codigo_barra
                            select u;
                if (!query.Any())
                {
                    ...
                    uds.un_telefono = !string.IsNullOrEmpty(telefono) ? telefono : "";
                    uds.un_direccion = !string.IsNullOrEmpty(direccion) ? direccion : "";
                    ...
                    if (id_lider > 0)
                        uds.id_lider = id_lider;
                    if (comodatario > 0)
                        uds.id_comodatario = comodatario;
                    if (bases > 0)
                        uds.id_base = bases;
                    if (estatus_instalacion > 0)
                        uds.estatus_instalacion = estatus_instalacion;
                    ...
                    DateTime fechaProgramada = tTiempo == 1 ? fecha_instalacion.AddDays(tMantenimiento * 7) : fecha_instalacion.AddMonths(tMantenimiento);
                    mtto.mtto_fecha_original = fecha_instalacion;
                    ...
                    b.bit_observaciones = ((si_usuarios)HttpContext.Current.Session["sesionUsuario"]).us_nombre + "(" + ((si_usuarios)HttpContext.Current.Session["sesionUsuario"]).us_user + ") - Usuario Agrego Nueva Unidades = " + codigo_barra;
                ...
                {
                    Response.Result = true;
                    Response.Message = "Ya Existe una unidad con este mismo número de contrato. ";
                    Response.Data = null;
                }
        ...
        [WebMethod(EnableSession = true)]
        public static ajaxResponse actualizar_u(string numero_economico, string numero_serie, int comodatario, int bases, int estatus_instalacion, DateTime fecha_instalacion, string codigo_barra,
            string telefono, string direccion, int anio, string placa, string observacion, int estatus, int id_empresa, int id_giro,
            int id_marca, int id_modelo, int id_persona, int id_cilindro, int id_lider, string vaporizador, string riel, string centralita, string observacionK,
            int? id_modeloK, int? id_cilindroKit, string[] numSerieT, DateTime[] fechaFT, string[] multvT, int[] marcaT, int[] tipoT, int[] capacidadT, int id, int tMantenimiento, int tTiempo)
        {
            ajaxResponse Response = new ajaxResponse();
            ...
            try
            {
                ...
                var cambioU = context.cambiosUnidades.Where(x => x.id_unidad.Equals(id));

                foreach (var detail in cambioU)
                {
                    context.cambiosUnidades.DeleteOnSubmit(detail);
                }
                context.SubmitChanges();


                cU.id_unidad = id;
                cU.fecha_cambio = DateTime.Now;
                ...
                else
                {
                    ...
                    for (int i = 0; i < numSerieT.Length; i++)
                    {
                        ...
                        tds.tq_numero_serie = !string.IsNullOrEmpty(numSerieT[i]) ? numSerieT[i] : "";
                        tds.tq_fecha_fabricacion = fechaFT[i].Year < 1950 ? DateTime.Now : fechaFT[i];
                        tds.tq_multivalvulas = !string.IsNullOrEmpty(multvT[i]) ? multvT[i] : "";
                        tds.tq_estatus = kds.ki_estatus;
                        if (marcaT[i] > 0)
                            tds.id_marca_tanque = marcaT[i];
